...
@mcp.tool()
async def switch_tab(session: str) -> str:
    """Back-compat convenience: make `session` the DEFAULT, so later tool calls that omit
    `session` act on this tab. In a concurrent swarm prefer passing `session=` explicitly
    rather than relying on the default (which any agent can change)."""
    page, refs, sid = _state.resolve(session)  # validates it exists
    _state._default = sid
    title = await page.title()
    return f"Default session is now {sid}: {title} ({page.url})"


@mcp.tool()
async def dump_cookies(path: str, domain: str = "") -> str:
    """Export the browser's current cookies (decrypted, from the live context) to a file in
    Netscape cookie-file format — the format yt-dlp / curl / requests want. Use this after
    logging into a site so other tools can reuse the session. `domain` optionally filters to
    cookies whose domain contains that substring (e.g. "instagram"). Returns a summary."""
    if not _state.context:
        return "Browser not open. open_browser, log in, then dump_cookies."
    import os as _os
    cookies = await _state.context.cookies()
    if domain:
        cookies = [c for c in cookies if domain.lower() in (c.get("domain", "").lower())]
    lines = ["# Netscape HTTP Cookie File",
             "# Generated by stealth-browser dump_cookies", ""]
    for c in cookies:
        dom = c.get("domain", "")
        ...
        exp = c.get("expires", 0)
        exp = 0 if (exp is None or exp < 0) else int(exp)
        name = c.get("name", "")
        val = c.get("value", "")
        lines.append(f"{dom}\t{include_sub}\t{cpath}\t{secure}\t{exp}\t{name}\t{val}")
    path = _os.path.expanduser(path)
    with open(path, "w") as f:
        f.write("\n".join(lines) + "\n")
    names = ", ".join(sorted({c.get("name", "") for c in cookies})[:12])
    return f"Wrote {len(cookies)} cookies to {path}" + (f" (domain~{domain})" if domain else "") + f". Names: {names}"


@mcp.tool()
async def get_links(session: str | None = None, contains: str = "", limit: int = 200) -> str:
    """Return all hyperlink hrefs (+ short visible text) on the current page as JSON — reads
    the actual <a href> values, which `snapshot` does NOT expose. Use to harvest reel/post URLs
    from a profile, hashtag, explore, search, or feed page where media show only as thumbnails.
    `contains` filters to hrefs containing a substring (e.g. "/reel/" or "/p/"). Results are
    deduped. Infinite-scroll pages load lazily — call `scroll` a few times, then get_links again
    and union the results to collect more."""
    page, _refs, _sid = _state.resolve(session)
    links = await page.evaluate(
        "() => Array.from(document.querySelectorAll('a[href]'))"
        ".map(a => ({href: a.href, text: (a.innerText||a.getAttribute('aria-label')||'').trim().slice(0,80)}))"
    )
    if contains:
        links = [l for l in links if contains in (l.get("href") or "")]
    seen, out = set(), []
    for l in links:
        h = l.get("href")
        ...
        if len(out) >= limit:
            break
    return json.dumps({"count": len(out), "links": out}, indent=2)
